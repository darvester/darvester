        ...
        try:
            while True:
                try:
                    ...
                    if e.status == 403:
                        logger.warning("403: Missing Access", exc_info=DEBUG)
                        logger.warning("403 returned working on guild %s", guildid)
                        continue
                    else:
                        raise e from e
            ...
            msg: str = ""
            if e.status == 429:
                msg = "HTTP 429 returned. You may have been temp banned." \
                      " Try again later (may take a couple hours or as long as a day)"
            logger.critical(msg or f"{e.status}: {e.text}", exc_info=DEBUG)
            self.close()
